 @foreach($categories as $cate)
  ...
  <?php $category = Session::has('locale') && Session::get('locale') == 'AR' && !empty($cate->arabic) ? $cate->arabic : $cate;
       ...
       $state_name = Session::has('locale') && Session::get('locale') == 'AR' && !empty($cate->country->arabic) ? $cate->country->arabic : $cate->country;
   ?>
                               <tr>
                                 <td width="50">
                           ...
                           <a class="btn btn-success btn-sm" href="{{route('admin.state.edit',[Session::get('locale'),$category->id])}}"><i class="fas fa-edit"></i></a>
                           
                                 
                                        
                                         <button class="btn btn-danger btn-sm btn-all-del"
                                          data-action="{{route('admin.state.delete',$cate->id)}}"
