...
.cstm404-container:after {
  content: "";
  background: linear-gradient(#00ccff, #ed1e7f);
  border-radius: 50%;
  position: absolute;
  top: 50%;
  left: 50%;
...
.cstm404-sec .bubble {
  background-image: linear-gradient(120deg, #54c9fc 0%, #e72681 100%);
  border-radius: 50%;
...
<div class="main">
  <h1>404</h1>
  <p>{{langText("It looks like you're lost...")}}<br/>{{langText("That's a trouble?")}}</p>
  <button type="button" onclick="goBack()" class="cstm-btn solid-btn">{{langText('Go back')}}</button>
</div>
</div>
</section>
    <!--   footer ends here -->
